...
function MostOrdered() {
  return (
    <section id="menu" className="mx-auto max-w-6xl px-6 py-16">
      <SectionTitle eyebrow="Most Ordered" title="The Fan Favorites" />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {mostOrdered.map((item) => (
          <article
            key={item.name}
            className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card p-6 transition hover:border-[color:var(--ember)]/60 hover:-translate-y-0.5"
          >
            ...
            <h3 className="relative text-2xl font-normal tracking-wide text-foreground">
              {item.name}
            </h3>
            {item.description && (
      ...
      <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4">
        {categories.map((c) => (
          <div
            key={c.name}
            className="rounded-2xl border border-dashed border-border/70 bg-card/40 p-5"
          >
            <div className="text-xs font-semibold uppercase tracking-[0.25em] text-[color:var(--accent)]">
              Category
            </div>
            <h4 className="mt-1 text-xl font-normal tracking-wide text-foreground">{c.name}</h4>
...
function ExtrasSides() {
  return (
    <section className="mx-auto max-w-6xl px-6 py-16">
      <SectionTitle eyebrow="Extras & Sides" title="Add To Your Order" />
      <div className="overflow-hidden rounded-2xl border border-border/60 bg-card">
        <ul className="divide-y divide-border/60">
          {extrasSides.map((item) => (
            <li
              key={item.name}
            ...
            >
              ...
              {item.price && (
                <div className="text-2xl font-normal tracking-wide text-gradient-flame">
                  {item.price}
                </div>
              )}
            </li>
